...
import { FiEdit3, FiUpload, FiMusic, FiActivity, FiHeart } from 'react-icons/fi';
...
export default function Profile() {
  ...
  const handleImageUpload = async (event) => {
    try {
      ...
      await currentUser.updateProfile({ photoURL }); // Update auth profile
  ...
  const handleProfileUpdate = async (data) => {
    try {
      ...
      await currentUser.updateProfile({ displayName: data.displayName }); // Update auth profile
      setIsEditing(false);
                ...
                {userProfile?.uploads?.length > 0 ? (
                  userProfile.uploads.map((upload) => (
                    <motion.div
                      ...
                      className="bg-dark/50 rounded-xl shadow-md p-4"
                    >
                      ...
                      <p className="text-sm text-gray-400">{upload.description}</p>
